...
void dieu_khien(int check,int x,int y){
  if(_kbhit()){
    ...
    if(c==-32){
            c=_getch();
        if(c==72&&check!=1) check=0,y--;//len tren.Dang len tren thi ko duoc xuong duoi
        else if(c==80&&check!=0) check=1,y++;//xuong duoi
        else if(c==75&&check!=3) check=2,x--;//sang trai
        else if(c==77&&check!=2) check=3,x++;//sang phai
